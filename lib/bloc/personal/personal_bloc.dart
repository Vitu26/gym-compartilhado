...
class PersonalBloc extends Bloc<PersonalEvent, PersonalState> {
  ...
  Future<void> _onPersonalLogin(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final decodedResponse = jsonDecode(response.body);
        final String? table = decodedResponse['table'];
        final Map<String, dynamic>? personalData = decodedResponse['data'];

        if (table == 'personals' && personalData != null) {
          await savePersonalLogado(
              personalData['id'].toString(), personalData['nome']);
          emit(PersonalSuccess(personalData)); // Sucesso com dados do personal
        } else {
          emit(
              const PersonalFailure('Erro: Estrutura de resposta inesperada.'));
        }
      } else {
        emit(
      ...
      emit(PersonalFailure('Login falhou: $e'));
    }
  }
  ...
  // Obtenção de todos os Personais
  Future<void> _onGetAllPersonais(
      GetAllPersonais event, Emitter<PersonalState> emit) async {
    emit(PersonalLoading());
    try {
      final token = await getToken();
      final response = await http.get(
  ...
  Future<void> _onDeletePersonal(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        emit(const PersonalSuccess('Personal deleted'));
      } else {
        emit(const PersonalFailure('Delete failed'));
      }
    } catch (e) {
  ...
  // // Atualização do perfil do personal
  // Future<void> _onUpdatePersonalProfile(
  //     UpdatePersonalProfile event, Emitter<PersonalState> emit) async {
  //   emit(PersonalLoading());
  //   try {
  ...
  Future<void> _onUpdatePersonalProfile(
    ...
    try {
      final String? token =
          await getToken(); // Supondo que você tenha uma função para obter o token

      // Verifica e imprime os dados que estão sendo enviados
      print('Dados enviados para atualização: ${event.updatedData}');
      ...
      if (response.statusCode == 200) {
        ...
        emit(PersonalFailure('Falha ao atualizar perfil: ${response.body}'));
      }
    } catch (e) {
      emit(PersonalFailure('Erro ao atualizar perfil: $e'));
    }
  }

  // Função para salvar o personal logado
  Future<void> savePersonalLogado(String id, String nome) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('personal_id', id);
    await prefs.setString('personal_nome', nome);
  ...
  // Função para obter o personal logado
  Future<void> _onGetPersonalLogado(
      GetPersonalLogado event, Emitter<PersonalState> emit) async {
    try {
      final personalData = await getPersonalLogado();
      emit(PersonalSuccess(personalData));
    } catch (e) {
      emit(PersonalFailure('Erro ao buscar personal logado: $e'));
    }
  }
