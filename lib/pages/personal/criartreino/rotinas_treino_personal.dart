...
class RotinasScreen extends StatelessWidget {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    onTap: () {
                      _checkIfRotinaHasTreinos(
                          context, rotina['id'].toString());
                    },
                  );
                },
              );
  ...
  // Função para verificar se a rotina possui treinos
    ...
    rotinaHasTreinoBloc.stream.listen((state) {
      if (state is RotinaHasTreinoLoaded) {
        if (state.rotinasHasTreinos.isNotEmpty) {
          // Navegar para a tela RotinaTreinosScreen se houver treinos
          Navigator.of(context).push(
            MaterialPageRoute(
              builder: (context) => RotinaTreinosScreen(rotinaId: int.parse(rotinaId)),
            ),
          );
        } else {
          // Navegar para a tela de adicionar treinos na rotina caso ela esteja vazia
          Navigator.of(context).push(
            MaterialPageRoute(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Erro ao verificar os treinos: ${state.error}')),
        );
      }
    });
  }
